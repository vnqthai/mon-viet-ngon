...
- **Cách kiểm nhanh cả site sau build** (chạy lại bất cứ lúc nào nghi ngờ): `grep -rhoE '(src|href)="https?://[^"]+"' dist --include='*.html' | sed -E 's/.*"(https?:\/\/[^/"]+).*/\1/' | sort | uniq -c` → chỉ được ra miền của mình; `git log --all --name-only --pretty=format: | grep -iE '\.(jpe?g|png|webp|gif|mp3|mp4|woff2?|otf|pdf|zip)$'` → lịch sử git cũng phải sạch (repo public thì lịch sử cũng đang phát hành).

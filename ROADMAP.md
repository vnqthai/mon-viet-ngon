...
- **Site không gọi ra một bên thứ ba nào.** Từ 2026-08-02 mọi byte khách tải đều từ chính miền này (font là bên-thứ-ba cuối cùng, đã tự host). Thêm bất cứ `<script src>`, `<link href>`, `<img src>`, `@import`, iframe hay CDN nào trỏ ra ngoài là **phá luật** — tài nguyên mới thì tải về repo, kiểm license, phục vụ từ chính mình. Ngoại lệ duy nhất được phép *cân nhắc*: GA4 (phần 2 mục 1) — quyết riêng, vì kéo theo cookie + dải xin phép.

...
| **QA** | `npm run qa` (bắt buộc trước mỗi build; từ 2026-08-02 gồm chốt chặn bên-thứ-ba/font/binary, và CI cũng tự chạy nó sau build trên mọi push) · `npm run link-audit` + `npm run seo-audit` (sau build) · `npm run art-png -- --sheet` · `npm run contact-sheet` |
...
1. ~~**Thống kê truy cập — Google Analytics 4**~~ — **GẮN XONG 2026-09-11** (Thái chốt GA4, không GoatCounter, từ 2026-07-30; property `G-DDLS1WV3MC` trong tài khoản GA có sẵn của Thái, múi giờ VN). **Cách gắn: "basic consent mode" — ô hỏi nhỏ một dòng ở góc dưới trái cho MỌI khách (Thái không muốn câu giải thích trên UI, cũng không muốn chữ "lượt xem": chỉ *"Cho bếp bật Google Analytics nhé?"* + Đồng ý · Không · Chi tiết); chưa bấm *Đồng ý* thì trang không tải một byte nào từ Google.** Hỏi mọi khách chứ không riêng EU vì Luật BVDLCN 91/2025 + NĐ 356/2025 (hiệu lực 01/01/2026) đòi xin phép TRƯỚC khi đặt cookie thống kê (DLA Piper, bản 02/2026: *"prior acceptance before … cookies … are activated"*). Loader ở `src/scripts/site.js`, chỉ chạy trên đúng `www.monvietngon.com` (dev/preview/harness không bao giờ bắn hit); lựa chọn lưu `mvn:consent`; bấm *Không* là xoá cookie `_ga*` và không hỏi lại; công bố + nút đổi ý ở `/gioi-thieu/#thong-ke` (điều 7 ToS GA đòi). **Còn tay Thái trong GA:** tắt *Page changes based on browser history events* (vì `/mon/` gọi `replaceState` mỗi lần bấm chip), retention 2 → 14 tháng, Google signals để tắt; kiểm Realtime bằng cửa sổ ẩn danh (máy mình bấm *Không*); nối Search Console (SEO.md §8 mục 5). Số để quyết hero C (mục 4): xem sau ~1 tuần có dữ liệu.
   ...
   >
   > **Đã trả lời cả ba lúc gắn (2026-09-11):** 145 KB chỉ tải SAU khi khách bấm Đồng ý, và `async`, nên trang của người không đồng ý vẫn đúng như cũ; ô xin phép là pill nhỏ nổi ở góc dưới trái, không chen vào bố cục hero (số đo hero giữ nguyên); GSC nối trong GA4 Admin → Product links, việc tay ở SEO.md §8 mục 5.
2. ~~**RSS feed**~~ — **xong 2026-07-30**, `/rss.xml`, 64 món sắp theo `pubDate` mới nhất trước.
3. ~~**Tự host font, bỏ Google Fonts**~~ — **XONG 2026-08-02**, làm trong đợt rà bản quyền: Thái muốn site không gọi ra bên thứ ba nào (IP khách sang Google là chuyện GDPR) — tức là quyết bằng lý do pháp lý, không cần chờ số bóp băng thông mà ghi chú cũ đòi. Cách làm đúng như ghi chú cũ dặn: KHÔNG dùng thẳng ba file `.ttf` (nặng hơn bản cắt), mà tải đúng **15 file woff2 Google đang phục vụ** (5 face × latin/latin-ext/vietnamese, ~150 KB cả bộ — cân nặng tải về y như cũ, chỉ đổi nguồn) về `public/fonts/`, kèm luôn hai face trước giờ thiếu bản local (Be Vietnam Pro 700, Dancing Script 600). Khai báo ở `src/styles/fonts.css`, giữ nguyên `unicode-range` + `font-display:swap`; license OFL đủ ba họ ở `src/assets/fonts/*-OFL.txt`.

   > **Kiểm theo đúng luật phần 4** — dựng hai bản hai cổng, `fonts.load()` đủ nét rồi mới đo, `loaded === 15` cả hai bên: chữ ký hình học 60 phần tử trang chủ + 80 phần tử trang phở bò **trùng khít tới 2 số lẻ** (hash hai bên như một: 3558064690 / 1997675274); `performance` của bản mới chỉ còn đúng một host là chính nó. Cùng ngày, `tools/contact-sheet.mjs` (đồ nghề dev, chỗ cuối cùng còn gọi CDN) cũng chuyển sang nhúng font local dạng data-URI — đọc thẳng `fonts.css` lúc sinh trang nên nâng bản font là tự khớp; sheet kiểm lại: 12/12 face nạp, 0 request ra ngoài.
...
**Tài sản & bên thứ ba** *(chốt 2026-08-02 sau đợt rà bản quyền — Thái yêu cầu: site không được tốn một xu, không được dính tài sản không-free, bây giờ và mãi về sau. MỌI việc sau này — thêm món, thêm trang, thêm tool, thêm dependency — đều phải soi qua nhóm luật này. Hiện trạng đã kiểm sạch toàn bộ + số đo ở phần 2 "Hạ tầng" mục 3.)*
- **Site không gọi ra một bên thứ ba nào.** Từ 2026-08-02 mọi byte khách tải đều từ chính miền này (font là bên-thứ-ba cuối cùng, đã tự host). Thêm bất cứ `<script src>`, `<link href>`, `<img src>`, `@import`, iframe hay CDN nào trỏ ra ngoài là **phá luật** — tài nguyên mới thì tải về repo, kiểm license, phục vụ từ chính mình. **Ngoại lệ duy nhất, đã chốt 2026-09-11, và có điều kiện: GA4 (phần 2 mục 1)** — đúng một URL `https://www.googletagmanager.com/gtag/js`, chỉ được chèn từ `src/scripts/site.js` SAU khi khách bấm *Đồng ý*; khách chưa bấm thì câu đầu bullet này vẫn đúng từng chữ. Không được đặt thẻ `<script src>` tĩnh cho nó (qa chặn), không được kéo thêm gtm.js hay thứ gì khác của Google. Còn `<a href>` trỏ ra ngoài là *liên kết để bấm*, không phải tài nguyên tự tải — không tính (qa bỏ qua thẻ `a`; hiện có đúng một, ở `/gioi-thieu/` trỏ tới trang giải thích của Google).
- **Tài sản vào repo phải free có giấy, xác minh ở CHÍNH FILE.** Font: chỉ OFL/tương đương, đọc **name table nhúng trong file** mà kiểm (đừng tin tên file — font thương mại đổi tên là bẫy kinh điển), và văn bản license commit cạnh font (`src/assets/fonts/*-OFL.txt`). Hình/icon: chỉ đồ tự vẽ trong repo (luật "Màu và hình") — không stock, không trace, không tải ảnh ngoài, kể cả đồ "free" (icon Font Awesome Free là CC-BY, thiếu attribution là dính). Nhạc/video: không có và đừng có.
- **Không dịch vụ nào có trang billing.** Không API key, không SaaS, không gói trả tiền. Dependency mới: license free (MIT/Apache/MPL/OFL/BSD…), chạy lúc build — đọc `license` trong `node_modules/<pkg>/package.json` trước khi thêm. Hạ tầng đứng nguyên GH Pages (vượt mức mềm là bị bóp tốc độ, không bị tính tiền) + domain của mình.
- **Chữ phải của mình.** Luật "Chính xác & trung thực" lo viết cho đúng; luật này thêm mặt kia: không chép nguyên văn của ai — nghi câu nào thì WebSearch **nguyên câu trong ngoặc kép**, không có kết quả trùng mới cho qua (cách đã dùng để kiểm 2026-08-02).
- **`npm run qa` TỰ CHẶN từ 2026-08-02** (`tools/check-third-party.mjs`), ba đường: ① tài nguyên trỏ ra host lạ trong nguồn + YAML + `dist/` (nếu đã build — chưa build thì nói to là đang bỏ qua phần đó); ② font khai ở `fonts.css` mà `public/fonts/` thiếu file wOF2 thật — thứ gãy **im lặng**: build vẫn xanh, trình duyệt lặng lẽ rơi về font hệ thống; ③ file nhị phân theo dõi trong git ngoài hai chỗ font hợp lệ — ảnh/nhạc lọt repo là vi phạm dù chưa trang nào tham chiếu. Chuỗi định danh (schema.org, w3.org) và URL nằm trong chú thích thì không tính. Đã thử đủ năm ca lúc dựng: chèn script lạ vào dist ✗ bắt · xoá 1 font ✗ bắt · URL sống trong code ✗ bắt · git add một .png ✗ bắt · URL trong chú thích ✓ cho qua. **Từ 2026-09-11 qa biết đúng một ngoại lệ** — URL gtag.js, chỉ trong `src/scripts/site.js` + file JS trong `dist/` — in số chỗ dùng ở dòng tổng kết; và bỏ qua `<a href>` ra ngoài (liên kết, không phải tài nguyên). Thử lại lúc sửa: gtag.js đặt vào YAML ✗ bắt · gtm.js trong site.js ✗ bắt · `<script src>` gtag tĩnh trong Base.astro ✗ bắt · `<link href>` ra ngoài ✗ bắt · `<a href>` ra ngoài ✓ cho qua.
- **CI giữ luật trên MỌI lần push** (cùng ngày): `deploy.yml` chạy `npm run qa` **sau** bước build (nên soi được cả `dist/` thật) — phạm luật là job build đỏ, job deploy không chạy. Tức là commit đẩy thẳng không qua qa ở máy mình cũng không lọt được lên site.
- **Kiểm tay khi nghi ngờ** (tham chiếu, qa đã tự làm phần đầu): `grep -rhoE '(src|href)="https?://[^"]+"' dist --include='*.html' | sed -E 's/.*"(https?:\/\/[^/"]+).*/\1/' | sort | uniq -c` → chỉ được ra miền của mình, cộng đúng một `https://policies.google.com` (liên kết `<a>` ở trang giới thiệu); gtag.js không nằm trong HTML vì site.js chèn lúc chạy — soi bằng `grep -rl googletagmanager src dist` phải ra đúng `src/scripts/site.js` + bundle JS của nó; `git log --all --name-only --pretty=format: | grep -iE '\.(jpe?g|png|webp|gif|mp3|mp4|woff2?|otf|pdf|zip)$'` → **lịch sử** git cũng phải sạch (repo public thì lịch sử cũng đang phát hành; qa soi file *đang* theo dõi, còn lịch sử cũ vẫn là việc tay — đã rà sạch toàn bộ 2026-08-02).
- **Font đông cứng ở bản tải 2026-08-02 — là chủ ý** (ổn định hơn mới). Muốn nâng bản: lấy CSS `fonts.googleapis.com/css2?family=Paytone+One&family=Be+Vietnam+Pro:wght@400;600;700&family=Dancing+Script:wght@600&display=swap` bằng User-Agent Chrome hiện hành, tải 15 file trong các `url(…)`, đặt tên `<họ>-<nét>-<subset>.woff2` vào `public/fonts/`, chép khối `@font-face` (giữ nguyên `unicode-range`) vào `src/styles/fonts.css` — tên file mới là tự phá cache. Xong dựng hai bản đo chữ ký hình học theo luật "Đo đạc".
- **Đồ nghề dev cũng sạch CDN** (2026-08-02): `tools/contact-sheet.mjs` nhúng font local dạng data-URI, đọc thẳng `src/styles/fonts.css` lúc sinh trang nên nâng bản font là tự khớp. URL `css2` còn xuất hiện trong tài liệu chỉ là *công thức nâng bản* (bullet trên), không phải request nào hết — repo không còn dòng CDN sống nào.
...
## Gắn Google Analytics 4 có chốt xin phép — 2026-09-11

**Chuyện gì:** Thái hỏi cách gắn GA để đo SEO. Tra lại luật trước khi làm thì ra một điều đổi cách gắn: **Luật Bảo vệ dữ liệu cá nhân 91/2025/QH15 + NĐ 356/2025** (hiệu lực 01/01/2026) đòi xin phép TRƯỚC khi bật cookie thống kê, và xếp dữ liệu theo dõi hành vi trên mạng vào nhóm nhạy cảm — tức là dải xin phép không còn là chuyện "nếu có khách EU" như ghi chú 07/2026 tính, mà cho **mọi khách**. Chọn cách sạch nhất: **basic consent mode** — chưa bấm *Đồng ý* thì trang không tải một byte nào từ Google; ai không bấm thì luật "mọi byte từ chính miền" vẫn đúng nguyên với họ. Google Analytics ToS điều 7 đòi có trang công bố ⇒ thêm mục "Thống kê lượt xem" ở `/gioi-thieu/` + nút đổi ý + link footer. Property `G-DDLS1WV3MC`, tài khoản GA sẵn có của Thái.

**Cách làm:** loader + dải hỏi trong `src/scripts/site.js` (lựa chọn `mvn:consent`; bấm *Không* là xoá cookie `_ga*` + bật cờ `ga-disable`), ô hỏi `#consent` trong `Base.astro` là pill một dòng nổi ở góc dưới trái — Thái không muốn câu giải thích trên UI, chỉ một câu hỏi + Đồng ý · Không · Chi tiết (không đụng bố cục hero — số đo hero giữ nguyên), chỉ đo trên đúng `www.monvietngon.com` nên dev/preview/harness không bao giờ bắn hit. `check-third-party.mjs` mở đúng MỘT ngoại lệ (URL gtag.js, chỉ trong site.js + bundle) và học phân biệt `<a href>` (liên kết để bấm) với tài nguyên tự tải.

**Cùng ngày, hai sự kiện tùy chỉnh** (GA4 tự không đếm bấm nút nội bộ): `hero_cta` (`cta` = `mon` | `bi-quyet`) trên hai nút hero trang chủ — cho quyết định phương án C (phần 2 mục 4); `filter_change` (`axis` = `cat` | `occ` | `region`, `value` = chip vừa chọn, `all` khi bỏ chọn) trên `/mon/` — cho câu hỏi thứ tự chip lọc. Cả hai chỉ gọi `window.gtag` nếu nó tồn tại, tức khách đã Đồng ý; không cần khai gì bên GA, sự kiện tự hiện ở Reports → Engagement → Events.

**Việc tay còn lại (Thái, trong GA):** tắt *Page changes based on browser history events* (`/mon/` gọi `replaceState` mỗi lần bấm chip lọc — bật thì mỗi cú bấm thành một page_view), retention 2 → 14 tháng (không hồi tố), Google signals tắt; kiểm Realtime bằng cửa sổ ẩn danh; nối GSC (SEO.md §8 mục 5). Hero C (phần 2 mục 4) chờ ~1 tuần số.

**Ba bài học:**
1. **Soi lại luật ở cả nước mình, không chỉ EU.** Ghi chú 07/2026 chỉ tính GDPR; luật VN mới hơn và chặt ngang. Tra trước khi làm rẻ hơn gỡ sau.
2. **"Không tải gì cho tới khi đồng ý" là cách duy nhất giữ được luật cũ nguyên văn** — consent mode "advanced" (tải sẵn, gửi ping không cookie) vẫn đưa IP khách sang Google trước khi hỏi, đúng chỗ án lệ fonts LG München đã phạt.
3. **Tool tĩnh phân biệt được "liên kết" với "tài nguyên", nhưng KHÔNG kiểm được "chỉ sau khi đồng ý"** — điều kiện đó sống trong site.js, nên sửa site.js là phải đọc lại chú thích đầu khối đó.

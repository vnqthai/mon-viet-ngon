...
| **QA** | `npm run qa` (bắt buộc trước mỗi build; gồm cả chốt chặn bên-thứ-ba/font từ 2026-08-02) · `npm run link-audit` + `npm run seo-audit` (sau build) · `npm run art-png -- --sheet` · `npm run contact-sheet` |
...
- **`npm run qa` TỰ CHẶN từ 2026-08-02** (`tools/check-third-party.mjs`): tài nguyên trỏ ra host lạ trong nguồn + YAML + `dist/` (nếu đã build — chưa build thì nói to là đang bỏ qua phần đó), và font khai ở `fonts.css` mà `public/fonts/` thiếu file wOF2 thật — thứ gãy **im lặng**: build vẫn xanh, trình duyệt lặng lẽ rơi về font hệ thống. Chuỗi định danh (schema.org, w3.org) và URL nằm trong chú thích thì không tính. Đã thử đủ bốn ca lúc dựng: chèn script lạ vào dist ✗ bắt · xoá 1 font ✗ bắt · URL sống trong code ✗ bắt · URL trong chú thích ✓ cho qua.
- **Kiểm tay khi nghi ngờ** (tham chiếu, qa đã tự làm phần đầu): `grep -rhoE '(src|href)="https?://[^"]+"' dist --include='*.html' | sed -E 's/.*"(https?:\/\/[^/"]+).*/\1/' | sort | uniq -c` → chỉ được ra miền của mình; `git log --all --name-only --pretty=format: | grep -iE '\.(jpe?g|png|webp|gif|mp3|mp4|woff2?|otf|pdf|zip)$'` → lịch sử git cũng phải sạch (repo public thì lịch sử cũng đang phát hành — cái này qa không soi, vẫn là việc tay).
